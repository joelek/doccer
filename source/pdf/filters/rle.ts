...
export const RLE = {
	...
	encode(decoded: Uint8Array): Uint8Array {
		let bytes = [] as Array<number>;
		let offset = 0;
		function encodeRawSequence(sequence: RawSequence): void {
		...
		function encodeRunSequence(sequence: RunSequence): void {
		...
		function appendRunSequence(sequence: RunSequence): void {
			if (queued_raw_sequence == null) {
