...
wtf.test(`VerticalLayoutNode should create one segment when there are no children.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({});
...
wtf.test(`VerticalLayoutNode should provide each child with the size left in the current segment.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({}, new MockNode({ w: 0, h: 1 }), new MockRemainingHeightNode());
    let atoms = node.createSegments({ w: 0, h: 10 }, { w: 0, h: 2 });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should pack the segments as densely as possible when the first segmentation point is before the first child.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({}, new MockNode({ w: 0, h: 4 }, { w: 0, h: 4 }), new MockNode({ w: 0, h: 4 }, { w: 0, h: 4 }));
...
wtf.test(`VerticalLayoutNode should support height.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ height: 10 });
...
wtf.test(`VerticalLayoutNode should support height "50%".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ height: "50%" });
...
wtf.test(`VerticalLayoutNode should support height "extrinsic".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ height: "extrinsic" });
    let atoms = node.createSegments({ w: 0, h: 10 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support height "intrinsic".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ height: "intrinsic" }, new MockNode({ w: 0, h: 10 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support overflow "hidden".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ overflow: "hidden" });
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support overflow "visible".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ overflow: "visible" });
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support segmentation "auto".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ segmentation: "auto" }, new MockNode({ w: 0, h: 2 }), new MockNode({ w: 0, h: 2 }));
    let atoms = node.createSegments({ w: 0, h: 10 }, { w: 0, h: 2 });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support segmentation "none".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ segmentation: "none" }, new MockNode({ w: 0, h: 2 }), new MockNode({ w: 0, h: 2 }));
    let atoms = node.createSegments({ w: 0, h: 10 }, { w: 0, h: 2 });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support width.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ width: 10 });
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support width "50%".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ width: "50%" });
...
wtf.test(`VerticalLayoutNode should support width "extrinsic".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ width: "extrinsic" });
    let atoms = node.createSegments({ w: 10, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support width "intrinsic".`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ width: "intrinsic" }, new MockNode({ w: 10, h: 0 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align x "left" when there is no horizontal overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_x: "left", width: 8 }, new MockNode({ w: 2, h: 0 }), new MockNode({ w: 2, h: 0 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align x "left" when there is horizontal overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_x: "left", width: 8 }, new MockNode({ w: 2, h: 0 }), new MockNode({ w: 10, h: 0 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align x "center" when there is no horizontal overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_x: "center", width: 8 }, new MockNode({ w: 2, h: 0 }), new MockNode({ w: 2, h: 0 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align x "center" when there is horizontal overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_x: "center", width: 8 }, new MockNode({ w: 2, h: 0 }), new MockNode({ w: 10, h: 0 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align x "right" when there is no horizontal overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_x: "right", width: 8 }, new MockNode({ w: 2, h: 0 }), new MockNode({ w: 2, h: 0 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align x "right" when there is horizontal overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_x: "right", width: 8 }, new MockNode({ w: 2, h: 0 }), new MockNode({ w: 10, h: 0 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align y "top" when there is no vertical overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_y: "top", height: 8 }, new MockNode({ w: 0, h: 2 }), new MockNode({ w: 0, h: 2 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align y "top" when there is vertical overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_y: "top", height: 8 }, new MockNode({ w: 0, h: 2 }), new MockNode({ w: 0, h: 10 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align y "middle" when there is no vertical overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_y: "middle", height: 8 }, new MockNode({ w: 0, h: 2 }), new MockNode({ w: 0, h: 2 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align y "middle" when there is vertical overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_y: "middle", height: 8 }, new MockNode({ w: 0, h: 2 }), new MockNode({ w: 0, h: 10 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align y "bottom" when there is no vertical overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_y: "bottom", height: 8 }, new MockNode({ w: 0, h: 2 }), new MockNode({ w: 0, h: 2 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support align y "bottom" when there is vertical overflow.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ align_y: "bottom", height: 8 }, new MockNode({ w: 0, h: 2 }), new MockNode({ w: 0, h: 10 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
...
wtf.test(`VerticalLayoutNode should support gap.`, (assert) => {
    let node = new vertical_1.VerticalLayoutNode({ gap: 1 }, new MockNode({ w: 0, h: 2 }), new MockNode({ w: 0, h: 2 }));
    let atoms = node.createSegments({ w: 0, h: 0 }, { w: 0, h: Infinity });
    assert.equals(atoms, [
        {
            "size": {
